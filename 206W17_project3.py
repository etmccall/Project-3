...
for tweet in umich_tweets: 
	z=tweet['entities']['user_mentions'] 
	if z !=[]: 
		r=z[0]['screen_name']
		q=api.get_user(r)
		m=q['favourites_count']
		s=q["description"]
		y=z[0]['id_str']
		ttt.append(y)
	else: 
		...
		y=tweet['user']['id_str']
		s=tweet['user']["description"]
		m=tweet['user']['favourites_count']
		ttt.append(y)
	if ttt.count(y) ==1: 
		tupz=(y, r, m, s)
...
table_spec += 'user_posted TEXT, time_posted TIMESTAMP, text TEXT, retweets INTEGER)'
cur.execute(table_spec)
...
for x in range(0,20): 
	tupz=(umich_tweets[x]["id"], ttt[x], umich_tweets[x]['created_at'], umich_tweets[x]["text"], umich_tweets[x]['retweet_count'])
	cur.execute(statement, tupz)
conn.commit()
...
wg="SELECT screen_name FROM Users"
cur.execute(wg)
wm=cur.fetchall()
screen_names=[]
for x in wm:
...
gg="SELECT description FROM Users where num_favs>5"
cur.execute(gg)
wmr=cur.fetchall()
...
mq="SELECT screen_name text FROM Tweets INNER JOIN Users on Tweets.user_posted=Users.user_id WHERE Tweets.retweets > 5";
cur.execute(mq)
...
description_words={x for m in descriptions_fav_users for x in m.split()}
...
z=collections.Counter(eme).most_common(3)

sorted_items = sorted(z, key = lambda x: x[0])
most_common_char=sorted_items[1][0]
...
for x in screen_names:
	g="SELECT text FROM Tweets WHERE instr(user_posted,x)";
	cur.execute(wg)
	sss=cur.fetchall()
	twee=[str(w[0]) for w in sss]
	twitter_info_diction[x]=twee
